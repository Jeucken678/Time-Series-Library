...
import torch
from torch.utils.data import Dataset  # 必须导入Dataset基类


class Dataset_RealEstate(Dataset):  # 正确继承Dataset
    def __init__(self, root_path, flag, seq_len, pred_len):
        # 初始化逻辑：加载数据、mask等
        self.root_path = root_path
        self.flag = flag
        ...
        # 加载数据（根据flag加载train/val/test数据）
        if flag == 'train':
            self.data = torch.load(f"{root_path}/train_data-17_nan.pt")
            self.nan_mask = torch.load(f"{root_path}/train_nan_mask-17_nan.pt")
            self.target_mask = torch.load(f"{root_path}/train_target_mask-17_nan.pt")
        elif flag == 'val':
            self.data = torch.load(f"{root_path}/val_data.pt")
            self.nan_mask = torch.load(f"{root_path}/val_nan_mask.pt")
            self.target_mask = torch.load(f"{root_path}/val_target_mask.pt")
        elif flag == 'test':
            self.data = torch.load(f"{root_path}/test_data.pt")
            self.nan_mask = torch.load(f"{root_path}/test_nan_mask.pt")
            self.target_mask = torch.load(f"{root_path}/test_target_mask.pt")

        # 确保数据形状正确（样本数 x 时间步）
        self.sample_num = self.data.shape[0]  # 样本数量

    def __getitem__(self, index):  # 必须实现，方法名不能错（__getitem__）
        # 根据索引index提取样本
        # 1. 提取输入序列（补充特征维度）
        seq_x = self.data[index, :self.seq_len].unsqueeze(-1)  # 形状：[T, 1]
        batch_y = seq_x  # 目标序列

        # 2. 提取mask（补充特征维度）
        nan_mask = self.nan_mask[index, :self.seq_len].unsqueeze(-1)  # [T, 1]
        target_mask = self.target_mask[index, :self.seq_len].unsqueeze(-1)  # [T, 1]

        # 3. 时间标记（示例）
        x_mark = torch.zeros((self.seq_len, 1))  # [T, 1]
        y_mark = torch.zeros((self.seq_len, 1))  # [T, 1]

        # 返回6个值（与exp_imputation.py的期望一致）
        return seq_x, batch_y, x_mark, y_mark, target_mask, nan_mask

    def __len__(self):  # 必须实现，返回样本总数
        return self.sample_num  # 样本数量由初始化时的data.shape[0]确定